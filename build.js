...
(async () => {
	try {
		...
		const source = await readFile('./dist/qb-report.js');

		searchStr = 'const deepmerge_1 =';
		searchRgx = new RegExp(searchStr);

		await writeFile('./dist/qb-report.prep.js', source.toString().replace(searchRgx, [
		...
		await writeFile('./dist/qb-report.browserify.js', browserified.outputText);

		console.log('Minify...');
		const results = await minify('./dist/qb-report.browserify.js');
		const license = await readFile('./LICENSE');

		searchStr = '1\\:\\[function\\(e,t,r\\)\\{\\(function\\(t\\)\\{\\"use strict\\";';
		searchRgx = new RegExp(searchStr);

		await writeFile('./dist/qb-report.browserify.min.js', results.toString().replace(searchRgx, [
		...
		console.log('Cleanup...');
		await exec([
			'rm ./dist/qb-report.prep.js',
			'rm ./dist/qb-report.browserify.js'
		].join(' && '));
